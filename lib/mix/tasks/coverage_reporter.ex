defmodule Mix.Tasks.CoverageReporter do
  @shortdoc "Analyzes coverage from lcov files and reports to GitHub"
  @moduledoc """
  Analyzes coverage from lcov files and reports to GitHub
  ...
  def run(args) do
    ...
    pull_number = Keyword.fetch!(opts, :pull_number)
    repository = Keyword.fetch!(opts, :repository)
    github_token = Keyword.fetch!(opts, :github_token)
    head_branch = Keyword.fetch!(opts, :head_branch)
  ...
  defp get_coverage_from_lcov_files do
    lcov_paths = Path.wildcard("cover/**/*lcov.info")
  ...
  defp get_changed_files(repository, pull_number, github_token) do
    {:ok, 200, files} = github_request_all("#{repository}/pulls/#{pull_number}/files", github_token)

    files
    |> Enum.reject(&String.equivalent?(&1["status"], "removed"))
    |> Enum.filter(&String.ends_with?(&1["filename"], ".ex"))
    |> Enum.map(fn file -> %{file: file["filename"], changed_lines: extract_changed_lines(file["patch"])} end)
  end

  defp create_module_results([], padding), do: "| No Changes | #{String.duplicate(" ", padding)} |"
  ...
  defp create_or_update_review_comment(repository, pull_number, summary, github_token) do
    {:ok, 200, reviews} = github_request_all("#{repository}/pulls/#{pull_number}/reviews", github_token)
  ...
  defp github_request(method, path, github_token, params) do
    ...
    request =
      case method do
        :get ->
          {'#{url}?#{URI.encode_query(params)}', headers}

        :post ->
          {url, headers, 'application/json', Jason.encode!(params)}
