defmodule CoverageReporter.UncoveredLineGrouper do
  ...
  Returns a list of line number groups, where each group represents
  consecutive uncovered lines that should be annotated together.
  """
  ...
  defp collect_group([{line_number, content, false, false} | rest], current_group) do
    case look_ahead_for_uncovered(rest, 1) do
      {true, _} ->
        collect_group(rest, [line_number | current_group])
  ...
  defp look_ahead_for_uncovered(lines, distance) do
    case {lines, distance} do
      ...
      # Skip one covered line and continue looking
      {[{_line_number, _content, false, false} | rest], distance} when distance < 1 ->
        look_ahead_for_uncovered(rest, distance + 1)
